    ...
    >
      <div className="flex flex-col md:flex-row items-center justify-between gap-12 lg:gap-24">
        ...
        <div className="md:w-1/2 w-full ">
          ...
          <button
            className="mt-8 bg-indigo-600 text-white px-8 cursor-pointer rounded-full 
                    px-8 py-3 hover:bg-indigo-700 transition-colors
                    "
          >
            Get Started
          </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
           {services.map((service, index) => (
            <div
              className="bg-white max-w-72 cursor-pointer rounded-2xl p-6 hover:scale-105 hover:shadow-xl duration-300 transition-all"
              key={index}
            >
                   <div className="mb-4">{service.icon}</div>
                   <h3 className="text-xl font-semibold mb-2 ">{service.title}</h3>
                   <p className="text-gray-600 mb-2">{service.description}</p>
                   <a className="" href={service.link} className="text-indigo-600 font-medium hover:text-indigo-700  transition-colors">Learn More</a>
            </div>
          ))} 
        </div>
      </div>
